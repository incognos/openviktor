...
Always presents drafts for human review before posting — use linkedin_publish_post to actually publish.`,
	input_schema: {
		...
		properties: {
			topic: {
				type: "string",
				description: "Topic, theme, or key message for the post (e.g. 'new partnership with X', 'thought leadership on MDM security', 'team milestone')",
			},
			post_format: {
				type: "string",
				enum: ["insight", "behind_scenes", "case_study", "hot_take", "how_to", "announcement", "question", "reactive"],
				description: "Post format. 'insight'=non-obvious domain insight, 'behind_scenes'=how we work, 'case_study'=customer outcome, 'hot_take'=industry position, 'how_to'=actionable tips, 'announcement'=milestone/launch, 'question'=audience engagement, 'reactive'=response to news. If unsure, omit and the tool will choose.",
			},
			brand_voice: {
				type: "string",
				enum: ["professional_friendly", "authoritative", "bold_direct", "thought_leader", "startup_energy"],
...
export function createLinkedInDraftPostExecutor(config: LinkedInConfig): ToolExecutor {
	return async (args): Promise<ToolResult> => {
		...
		const voiceGuide: Record<string, string> = {
			professional_friendly: "Warm, approachable, knowledgeable. Use 'we' and 'our team'. Avoid jargon. Celebrate people.",
			authoritative: "Data-driven, confident. Cite sources where possible. Short declarative sentences. Position as the trusted expert.",
			bold_direct: "Short punchy sentences. Contrarian if warranted. No hedging. Take a clear position.",
			thought_leader: "Share original insight, challenge conventional wisdom, invite debate. End with an open question.",
			startup_energy: "Behind-the-scenes, authentic, fast-moving. Celebrate milestones and lessons from failures.",
		};

		const formatGuide: Record<string, string> = {
			insight: "Share a non-obvious insight. Open with the surprising truth, explain it, give a takeaway.",
			behind_scenes: "Show how your team works. Make it human and specific — name things, give context.",
			case_study: "Tease a customer outcome with a specific result. Open with the problem, hint at the solution, share the number.",
			hot_take: "Take a clear position on an industry debate. State the conventional wisdom, then flip it.",
			how_to: "Give 3-5 actionable tips. Use a numbered list. Each point should be immediately usable.",
			announcement: "Lead with the news. Then explain why it matters. Keep it punchy — details go in comments or a linked post.",
			question: "Ask something your audience will have an opinion on. Make it easy to answer in one line.",
		...
		return {
			output: {
				...
				next_step: config.requireApproval !== false
...
export const linkedInGetRecentPostsDefinition: LLMToolDefinition = {
	...
	description: "Get the most recent posts from your company LinkedIn page. Useful for reviewing what was recently published before drafting new content.",
	input_schema: {
		type: "object",
		properties: {
			count: {
		...
		return {
			output: {
				posts: elements.map((p) => {
					...
					const shareContent = content?.["com.linkedin.ugc.ShareContent"] as Record<string, unknown> | undefined;
					const text = (shareContent?.shareCommentary as Record<string, unknown> | undefined)?.text ?? "";
					return {
						id: p.id,
						text: typeof text === "string" ? text.slice(0, 300) + (text.length > 300 ? "..." : "") : "",
						created: p.created,
