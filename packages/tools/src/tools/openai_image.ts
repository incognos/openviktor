...
export const openaiGenerateImageDefinition: LLMToolDefinition = {
	...
	description: "Generate an image using OpenAI gpt-image-1. Good for social media graphics, illustrations, and creative visuals with accurate text rendering.",
	input_schema: {
		type: "object",
		properties: {
			prompt: { type: "string", description: "Detailed description of the image to generate" },
			size: { type: "string", enum: ["1024x1024", "1536x1024", "1024x1536"], description: "Image dimensions (default: 1024x1024). Use 1536x1024 for landscape, 1024x1536 for portrait/LinkedIn." },
...
export function createOpenAIGenerateImageExecutor(config: OpenAIImageConfig): ToolExecutor {
	return async (args): Promise<ToolResult> => {
		...
		if (!res.ok) return { output: null, durationMs: 0, error: `OpenAI ${res.status}: ${JSON.stringify(data)}` };
