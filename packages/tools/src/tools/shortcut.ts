	...
	return async (args): Promise<ToolResult> => {
		...
		if (!r.ok)
			return {
				output: null,
				durationMs: 0,
				error: `Shortcut ${r.status}: ${JSON.stringify(r.data)}`,
			};
...
export function createShortcutSearchStoriesExecutor(config: ShortcutConfig): ToolExecutor {
	return async (args): Promise<ToolResult> => {
		...
		const r = await scFetch(
			config,
			`/search/stories?query=${encodeURIComponent(args.query as string)}&page_size=${limit}`,
		);
		if (!r.ok)
			return {
				output: null,
...
export const shortcutCreateStoryDefinition: LLMToolDefinition = {
	...
	input_schema: {
		...
		properties: {
			...
			workflow_state_id: {
				type: "number",
				description: "Workflow state ID (use shortcut_list_workflows to get IDs)",
			},
...
export const shortcutListWorkflowsDefinition: LLMToolDefinition = {
	...
	description:
		"List all Shortcut workflows and their state IDs. Use this to get workflow_state_id values.",
...
export function createShortcutListWorkflowsExecutor(config: ShortcutConfig): ToolExecutor {
	return async (): Promise<ToolResult> => {
		...
		const workflows = (r.data as Array<Record<string, unknown>>).map((w) => ({
			id: w.id,
			name: w.name,
			states: (w.states as Array<Record<string, unknown>>).map((s) => ({
				id: s.id,
				name: s.name,
				type: s.type,
			})),
